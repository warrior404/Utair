...
for oio in oio_dct.keys():
    ...
    tbl_sp = sp.findChildren('table')[7]

    # Цикл создания словаря
    rows = tbl_sp.findChildren(['th', 'tr'])
    c2 = 0
    uic_lst = []
    value_lst = []
    for row in rows:
        c2 += 1
        if c2 == 21:
            cell_lst = row.findChildren('td')
            for cell in cell_lst:
                uic_lst.append(cell.text)
        if c2 == 39:
            cell_lst = row.findChildren('td')
            for cell in cell_lst:
                value_lst.append(cell.text.strip().split('\n')[1])
